...
async function mapPeers(symbol: string, depth: number, result: peersResult = {}): Promise<peersResult> {
    ...
    try {
        ...
        //
        symbolPeers.forEach(
            async function (peer: string) { await mapPeers(peer, depth - 1, result); }
        );
        //
    } catch {
    }
...
export async function generatePeersGraph(symbol: string, depth: number): Promise<any> {
    // const p: peersResult = await mapPeers(symbol, depth);
    const graph: any = {
        "nodes": [
            {
                "id": "0",
